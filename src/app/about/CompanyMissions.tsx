...
export default function CompanyMissions() {
  ...
  return (
    <div className="px-12.5">
      <div className="bg-ikwp-orange-1 rounded-4xl p-12.5 grid grid-cols-5 gap-12.5">
        {missions.map((mission, index) => (
          <div 
            key={index} 
            className="bg-white rounded-4xl flex flex-col text-center items-center py-7 px-6"
            style={{ boxShadow: '8px 15px 25px rgba(0,0,0,0.16)' }}
          >
            <mission.icon className="w-45 h-45" style={{ color: "ikwp-orange-2" }}/>
            <div className="text-ikwp-orange-2 text-[28px] font-semibold">
              {mission.title}
            </div>
          </div>
        ))}
